...
export default forwardRef((props: any, ref: any) => {
  ...
  const { dataSource = [], defaultName= [] } = props
    ...
    () => ({
        show: ({ title= '', row }: any) => {
          ...
          if (row) {
            ...
            const api = defaultName.includes(row.name)? `/etc/keentune/profile/${row.name}`: `/var/keentune/profile/${row.name}`
            getRequestData(api)
          }
        }
    })
  ...
  const getFormData = () => {
    ...
    form.validateFields().then(async ( values ) => {
      ...
      const q = (title === 'edit' && name !== rowName) ? { ...values, name:`${name}.conf`, replace: `${rowName}.conf` }: {...values, name:`${name}.conf`}
  ...
  const label_Target = (
	...
	return (
      <Modal
        ...
        } // <ExclamationCircleOutlined style={{ fontSize:14,color:'#008dff'}}/>
        visible={visible}
        maskClosable={false}
        width={900}
